...
import { navHover, navText, navUnderline } from "@/lib/portfolioViewStyles";
      ...
      <nav className="hidden gap-3 md:flex lg:gap-5">
        {sections.map((section) => (
          <button
            ...
            className={`relative pb-2 text-[10px] transition lg:text-[11px] ${navHover(viewMode)}`}
          >
            {section.title}
            {activeSectionId === section.id ? (
              <motion.span
                layoutId="active-section-underline"
                className={`pointer-events-none absolute bottom-0 left-0.5 right-0.5 h-[3px] rounded-full ${navUnderline(viewMode)}`}
